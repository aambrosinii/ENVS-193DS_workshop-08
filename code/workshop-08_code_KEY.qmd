...
model1 <- lm(
  total_g ~ sla + water_treatment + species_name, # predictors added in formula
  data = drought_exp_clean # data frame
)
...
plot(model6)
plot(model7)
# all residuals look okay and no outliers
```
...
AICc(model1,
     model2,
     model3,
     model4, # best (lowest) model: water and species
     model5,
     model6,
     model7,
     model0) |>
...
Te best model that predicts total plant mass (g) includes water treatment and species (F(7, 62) = 27.08, p < 0.001, $\alpha$ = 0.05), but not SLA.  
...
Across the water treatments, all species have different (and higher) masses than _Lotus scoparius_ except _Penstemon centranthifolius_.
